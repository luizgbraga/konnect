...
@WebServlet(name = "graph-api", value = "/api/graph")
public class GraphAPI extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        for (ConnectsTo conn : connections) {
            String usernameFrom = "";
            String usernameTo = "";
            for (User user : users) {
                if (user.getId().equals(conn.getUserToId())) {
                    usernameTo = user.getUsername();
                }
                if (user.getId().equals(conn.getUserFromId())) {
                    usernameFrom = user.getUsername();
                }
            }
            responseBuilder.append("{")
                    .append("\"usernameFrom\": \"").append(usernameFrom).append("\", ")
                    .append("\"usernameTo\": \"").append(usernameTo).append("\"")
                    .append("}, ");
        }
        if (!connections.isEmpty() && responseBuilder.length() > 1) {
